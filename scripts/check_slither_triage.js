...
let reportsDir = process.env.SLITHER_OUTPUT_DIR || "/tmp/qbitmarket-contract-slither";
let allowlistPath = path.join(rootDir, "docs", "slither-accepted-findings.json");
let outputPath = "";
...
for (let index = 0; index < args.length; index += 1) {
  ...
  if (arg === "--reports-dir") {
    ...
    console.log("Usage: scripts/check_slither_triage.js [--reports-dir path] [--allowlist path] [--output path]");
    process.exit(0);
  } else {
...
console.log("All Slither findings are classified by docs/slither-accepted-findings.json");
