...
let outputPath = path.join("/tmp", "qbitmarket-contracts-v3-audit-evidence", "audit-evidence-manifest.json");
let summaryPath = path.join("/tmp", "qbitmarket-contracts-v3-audit-evidence", "audit-evidence-summary.md");
...
for (let index = 0; index < args.length; index += 1) {
  ...
  if (arg === "--output") {
    ...
    console.log("Usage: scripts/generate_contracts_v3_audit_manifest.js [--output path] [--summary-output path]");
    process.exit(0);
  } else {
...
const slitherOutputDir = path.resolve(process.env.SLITHER_OUTPUT_DIR || "/tmp/qbitmarket-contracts-v3-slither");
const mythrilOutputDir = path.resolve(process.env.MYTHRIL_OUTPUT_DIR || "/tmp/qbitmarket-contracts-v3-mythril");
...
const sourceFiles = [
  ...
  ...existing([
    "scripts/run_contracts_v3_slither.sh",
    "scripts/run_contracts_v3_mythril.sh",
    "scripts/check_contracts_v3_slither_triage.js",
    "scripts/generate_contracts_v3_audit_manifest.js",
    "docs/contracts-v3-audit-triage.md",
    "docs/contracts-v3-slither-accepted-findings.json",
    "docs/contracts-v3-operational-authority.md",
    "docs/contracts-v3-roadmap.md",
    "docs/contracts-v3-publication-model.md",
  ]),
];
