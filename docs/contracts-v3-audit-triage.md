...
```sh
SLITHER_ALLOW_FINDINGS=1 scripts/run_contracts_v3_slither.sh
```
...
- JSON artifacts are written to `/tmp/qbitmarket-contracts-v3-slither` by
...
- `docs/contracts-v3-slither-accepted-findings.json` is the source of truth for
  accepted or regression-tested Slither detector families.
- `scripts/check_contracts_v3_slither_triage.js` fails CI when a Slither report
...
- JSON reports and extracted bytecode are written to
  `/tmp/qbitmarket-contracts-v3-mythril` by default; override with
  `MYTHRIL_OUTPUT_DIR=...`.
